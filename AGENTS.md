# AGENTS.md — asset-dedup-pdf-page-extract

## Project Overview

PDF-extraction extension for `asset-dedup-core`.
Given one or more PDF paths on a shared volume, renders selected pages per PDF and writes each as a PNG file on that same volume — nothing else.
Hashing, combining per-page hashes, and calling back to core are explicitly out of scope; core sends the extracted page paths to `image-hash` itself afterward.
...
Requires `pdftoppm`/`pdfinfo` on `PATH` (or `PDFTOPPM_BIN`/`PDFINFO_BIN` pointing at them) for `npm run dev`, `test:unit`, `test:integration` — the PDF-processing path shells out to them directly against real fixture PDFs in `test/fixtures/`, nothing is mocked at the OS level.
...
- **Socket wire format is `snake_case`** (`op`, `page_selection`, `dpi`), internal TS is `camelCase` — map at the dispatch boundary (`extract.socket.ts`), never a raw pass-through of an internal camelCase shape.
- **Config**: env-only, zod-validated before use, grouped into one `Env` value (`infrastructure/env/env.ts`) — business logic depends on `env.PDFTOPPM_BIN` etc., not on scattered `process.env` reads; no YAML file, no `CONFIG_PATH`.
...
- **Singletons**: `pdfExtractor`, `idGenerator`, `pdfExtractService`, `ttlSweeper` are container singletons; `env` and `logger` are registered via `asValue`.
...
`pdftoppm`/`pdfinfo` (poppler-utils, GPL-2.0-or-later) run as separate OS processes (CLI args + `stdout`/`stderr`), never linked in — see `docs/THIRD-PARTY-NOTICES.md` before changing invocation.
...
- Split is not mock-vs-real-I/O — a unit test can touch real I/O if that's a detail of the one module under test.
  - **unit/** — exercises exactly one module. `PdfProvider`'s own unit tests run real `pdftoppm`/`pdfinfo` against fixture PDFs in `test/fixtures/` (including small fake-binary test doubles in `test/fixtures/bin/` for deterministic failure modes) — nothing is mocked at the OS level, see "Commands" above.
