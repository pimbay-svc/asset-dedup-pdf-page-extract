...
describe('buildContainer', () => {
  ...
  it('resolves pdfExtractService as a PdfExtractService wired to a real PdfProvider', async () => {
    dir = await mkdtemp(path.join(tmpdir(), 'container-test-'));

    const env = makeEnv({
      NODE_ENV: 'test',
      LOG_LEVEL: 'silent',
      SOCKET_PATH: path.join(dir, 'pdf-page-extract.sock'),
    ...
    expect(cradle.pdfExtractService).toBeInstanceOf(PdfExtractService);
    expect(cradle.pdfExtractor).toBeInstanceOf(PdfProvider);
    expect(cradle.env.OUTPUT_DIR).toBe(path.join(dir, 'pdf-page-extract'));
  });
