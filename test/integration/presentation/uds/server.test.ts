...
describe('UDS server', () => {
  ...
  it('handles a second request on the same already-active connection', async () => {
    ...
    socket.write(encodeFrame({ op: 'extract', config: { page_selection: 'first-middle-last', dpi: 150 }, inputs: {} }));
    await firstResponse;
  ...
  it('rejects data from a second connection while one is already active', async () => {
    ...
    first.socket.write(
      encodeFrame({ op: 'extract', config: { page_selection: 'first-middle-last', dpi: 150 }, inputs: {} }),
    );
  ...
  it('survives a structurally malformed extract message instead of crashing the process', async () => {
    ...
    socket.write(encodeFrame({ op: 'extract', inputs: { id1: { path: '/shared/a.pdf' } } }));
    await new Promise((resolve) => setTimeout(resolve, 20));
