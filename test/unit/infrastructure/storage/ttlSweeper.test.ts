...
// that I/O to settle. A short real interval plus a short real wait is simpler here.
...
describe('TtlSweeper', () => {
  ...
  it('logs a warning and continues if a single entry fails to process (e.g. unlink on a directory)', async () => {
    // Stale sub-directory: stat() succeeds (old enough to remove) but unlink() on a directory
    // fails — exercises the per-entry catch without affecting the rest of the sweep.
