...
/**
 * Accepts a single persistent connection from `core`, reconnecting on drop. A connection becomes
 * "active" only on its first valid frame — not on accept — so the Docker healthcheck's silent
 * connect-and-close probe can't be mistaken for `core` and block a real connection.
 */
export async function buildUdsServer(cradle: Cradle): Promise<UdsServerHandle> {
  ...
  server.on('connection', (socket) => {
    ...
    socket.on('data', (chunk: Buffer) => {
      ...
      /* v8 ignore next 3 -- decoder is always set synchronously above; guard is defensive only. */
      ...
      // Stryker disable next-line ConditionalExpression: guard above ensures activeSocket is
      // null or === socket here, so forcing the right operand true is an unobservable no-op.
    ...
    socket.on('close', () => {
      ...
      // delete only delays GC of the WeakMap entry — no observable effect.
